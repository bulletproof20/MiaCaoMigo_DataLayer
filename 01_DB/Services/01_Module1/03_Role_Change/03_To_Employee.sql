...
drop function if exists fn_alter_employee_to_assistant(
    int, varchar, int
);
...
begin
    ...
    if !fn_is_veterinarian(v_id_emp_old) and !fn_is_assistant(v_id_emp_old) then
        raise exception using
            message = 'Employee account with id ' || v_id_emp_old || ' is already a general role employee.';
    end if;
