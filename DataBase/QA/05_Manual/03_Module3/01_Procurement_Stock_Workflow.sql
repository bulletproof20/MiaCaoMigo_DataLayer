...
-- =========================================================
-- PURPOSE:
--   Ensure QA-PRO-001 exists, create pending purchase, fn_receive_purchase,
...
begin
    ...
    if not exists (select 1 from product where ref_pro = 'QA-PRO-001') then
        ...
        values ('QA-PRO-001', '9000000000001', 'Manual Integrity Product', 'QA', 14.50, 6.00, v_fam, 5)
        returning id_pro into v_pro;
    else
        select id_pro into v_pro from product where ref_pro = 'QA-PRO-001';
    end if;
...
select * from fn_get_product_stock_level(
    (select id_pro from product where ref_pro = 'QA-PRO-001')
);

select v.*
from fn_list_product_stock_levels() v
where v.ref_pro = 'QA-PRO-001';
